...
async function settingsFromAssets(request, env) {
  try {
    const response = await env.ASSETS.fetch(new Request(new URL("/settings.json", request.url)));
...
export default { async fetch(request, env) {
  ...
  if (incoming.pathname !== "/api/proxy") return env.ASSETS.fetch(request);
  ...
  for (const origin of origins) {
    ...
    try {
      const candidateHeaders = new Headers(headers); candidateHeaders.set("Host", target.host);
      upstream = await fetch(new Request(candidate, { method: request.method === "HEAD" ? "HEAD" : "GET", headers: candidateHeaders, redirect: "follow" }), { cf: { cacheEverything: true, cacheTtl: 3600 } });
      if (upstream.status >= 200 && upstream.status < 400) break;
    } catch {}
  }
