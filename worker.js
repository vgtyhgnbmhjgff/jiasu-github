...
function rewriteSrcset(value, incoming) { return value.split(",").map((part) => { const match = part.trim().match(/^(\S+)(.*)$/); if (!match) return part; try { const url = new URL(match[1], "https://github.com"); return allowedResource(url) ? `${resourceProxy(url.href, incoming)}${match[2]}` : part; } catch { return part; } }).join(", "); }
class ResourceElementHandler { constructor(incoming) { this.incoming = incoming; } element(element) { for (const name of ["src", "poster", "data-src", "data-canonical-src", "data-url"]) { const value = element.getAttribute(name); if (!value) continue; try { const url = new URL(value, "https://github.com"); if (url.hostname === "github.com" && ALLOWED_PATH.test(url.pathname)) url.pathname = url.pathname.replace("/blob/", "/raw/"); if (allowedResource(url)) element.setAttribute(name, resourceProxy(url.href, this.incoming)); } catch {} } const srcset = element.getAttribute("srcset"); if (srcset) element.setAttribute("srcset", rewriteSrcset(srcset, this.incoming)); } }
